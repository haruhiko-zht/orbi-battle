...
- Phaser シーンに攻撃射程サークルを追加し、生存状態に応じた透明度でデバッグ視認性を向上
- ファイター本体の半径を 6px に縮小し、新しい射程表示とのバランスを最適化
- デフォルトプリセット（1v1 / 3v3 / AI Demo / Mixed AI）で射程値を概ね 1/4（9〜10px）に再設定し、表示とシミュレーション挙動を同期
- `npm run test` を実行し、既存テストがすべてパスすることを確認
