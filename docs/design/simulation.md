...
- **FighterParams / FighterState** — ファイターのパラメータと現在値。`aiType` が未指定なら `nearest` を自動適用。
...
- **Validation** — `validation.ts` が入力構成を検証し、未知の AI や異常値を即座に弾きます。

## 進行ロジック（1 フレーム）

1. 存命ファイターを列挙し、`aiType` に応じてターゲットと移動方向を決定。
...
| `aiType`   | 戦略 ID。                           | `AI_DEFINITIONS` に定義されたもののみ有効。 |

## AI レジストリ

- 単一のソース・オブ・トゥルースは `src/sim/ai/types.ts` の `AI_DEFINITIONS`。
- 実装は `src/sim/ai/index.ts` の `aiFactories` に登録し、UI は `AI_OPTIONS` を参照します。
- 追加手順: `AI_DEFINITIONS` → `aiFactories` → 必要ならプリセット更新 → `__tests__/ai.test.ts` で決定論を確認。
